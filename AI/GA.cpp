...
public:
    ...
    void Run(){
        ...
        cout<<"价值总和最大为: "<<max_single.fit<<endl;
        cout<<"取得最大总和时的基因为: "<<endl;
        for(int i = 0; i < NUM; i++) {
            cout<<max_single.gene[i];
            if(i != NUM-1)
                cout<<" ";
        }
        cout<<endl<<"在第"<<max_single._count<<"代时取得最大总和。"<<endl;
    }
};
void GA::Init(){
    int i,j,wsum,vsum;
    for(i = 0;i < PS;i++){
        ...
        for(j = 0;j < NUM;j++){
            zq[i].gene[j] = ran();
            wsum += zq[i].gene[j]*weight[j];
            vsum += zq[i].gene[j]*c[j];
            }
